...
export const ProposalFields = () => {
    ...
    const updateTier = (
        id: string,
        patch: Partial<(typeof document.pricingTiers)[0]>
    ) => {
        updateDocument({
                ...
                <div className="space-y-2">
                    ...
                    <div className="border-l-4 border-violet-200 bg-violet-50/40 rounded-r-xl p-4">
                        <textarea
                            ...
                            placeholder={tr(
                                "Describe the full scope of work for this project…"
                            )}
            ...
            <div className="space-y-4">
                ...
                >
                    {(document.pricingTiers ?? []).map((tier) => (
                        <div
                            key={tier.id}
                            className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3 group/tier relative"
                        >
                            {/* Delete tier button */}
                            <button
                                onClick={() => removeTier(tier.id)}
                                className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-red-50 border border-red-200 text-red-400 hover:bg-red-500hover:text-white transition-all flex items-center justify-center md:opacity-0 group-hover/tier:opacity-100"
                                title="Remove package"
                            >
                                <TrashIcon
                                    size={10}
                                    className="md:text-slate-300 md:hover:text-red-400 text-red-400"
                                />
                            </button>
            ...
            {(!hiddenFields.includes("defaultCurrency") ||
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {!hiddenFields.includes("defaultCurrency") && (
                        <FormField
                            label="Currency"
                            onDelete={() => hideField("defaultCurrency")}
                        >
                            <input
                                type="text"
                                className={inputClass}
            ...
            <div className="space-y-2">
                ...
                <div className="border border-slate-200 rounded-xl overflow-hidden divide-y divide-slate-200">
                    {document.deliverables.map((row, idx) => (
                        <div
                            key={row.id}
                            className="flex gap-4 items-start p-4 group/del bg-white hover:bg-slate-50/60 transition-colors"
                        >
                            {/* Numbered bullet */}
